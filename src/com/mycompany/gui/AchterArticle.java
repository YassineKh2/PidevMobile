...
import com.mycompany.services.ServiesArticles;
 ...
 * @author yassine
 */
public class AchterArticle extends BaseForm {

    Form current;

    public AchterArticle(Resources res, Article Art) {
        ...
        Label ArticleNom = new Label("Nom : " + Art.getNomArticle(), "NewsTopLine");
        Label ArticlePrix = new Label("Prix : " + Art.getPrixArticle().toString() + "DT", "NewsTopLine");
        Label ArticleDis = new Label(Art.getArticleDiscription(), "NewsTopLine");
        TextField ArticleQuantite = new TextField("", "Quantite a achter");
        ...
        add(content);
        if (Art.getQuantiteArticle() > 0) {
            add(ArticleQuantite);
            Button btnAchter = new Button("Achter");
            addStringValue("", btnAchter);

            btnAchter.addActionListener(l -> {
                String quantite = ArticleQuantite.getText();

                try {
                    if ("".equals(quantite)) {
                        Dialog.show("Veuiillez verfier les données", "", "Annuler", "Ok");
                    }
                    int quantiteInt = Integer.valueOf(quantite);
                    if (quantiteInt > Art.getQuantiteArticle()) {
                        Dialog.show("Quantite plus grande que stock", "", "Annuler", "Ok");
                    } else {
                        Dialog.show("Achté !", "", "Annuler", "Ok");
                        Art.setQuantiteArticle(Art.getQuantiteArticle() - quantiteInt);
                        System.out.println(Art.getId());
                        boolean test = ServiesArticles.getInstance().ModifyArticleQuantite(Art);
                        Categorie categ = new Categorie();
                        categ = ServicesCategorie.getInstance().GetSingleCategorie(Art.getIdCategorie(), categ);
                        new ListArticle(res, categ).show();
                        refreshTheme();
            ...
            Label HorsStock = new Label("Article Hors stock !");
            add(HorsStock);
        }
